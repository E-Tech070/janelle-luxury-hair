var WHATSAPP = "2348107796481";
var qty = 1;
var PRODUCT_API = "http://localhost:5000/api/products";
...
function loadProduct() {
  ...
  fetch(PRODUCT_API + "/" + id)
    ...
    .then(function(product) { renderProduct(product); })
    .catch(function() {
      var fallback = null;
      if (typeof PRODUCTS !== "undefined") {
        for (var i = 0; i < PRODUCTS.length; i++) {
          if (PRODUCTS[i].id === id) { fallback = PRODUCTS[i]; break; }
        }
      }
      if (fallback) renderProduct(fallback);
      else {
        document.querySelector(".product-container").innerHTML = '<p style="color:#888;text-align:center;grid-column:1/-1;padding:4rem;">Product not found. <a href="index.html#shop" style="color:#c9a84c;">Back to shop</a></p>';
      }
    });
}
...
function renderProduct(product) {
  ...
  document.getElementById("product-img").src = urlImg ? decodeURIComponent(urlImg) : product.img;
  document.getElementById("product-img").alt = product.name;

  if (product.badge) {
    document.getElementById("product-badge-display").textContent = product.badge;
    document.getElementById("product-badge-display").style.display = "inline-block";
  }

  document.getElementById("product-category-display").textContent = product.category;
  ...
  var waText = encodeURIComponent("Hi Janelle! I am interested in the " + product.name + " (" + product.price + "). Please confirm availability.");
  document.getElementById("whatsapp-order-btn").href = "https://wa.me/" + WHATSAPP + "?text=" + waText;
  ...
  document.getElementById("add-to-cart-btn").addEventListener("click", function() {
    addToCart(product.name, product.price, urlImg ? decodeURIComponent(urlImg) : product.img, qty);
    ...
    setTimeout(function() {
      ...
      document.getElementById("add-to-cart-btn").innerHTML = '<i class="bi bi-bag-plus"></i> Add to Cart';
    }, 3000);
  });
}
