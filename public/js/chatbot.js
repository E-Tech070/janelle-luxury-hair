...
// ============================================================
// LOCAL FALLBACK — used whenever the real AI call fails (e.g. no
// API credit yet, or a network hiccup). Keeps the chatbot always
// answering something useful instead of an error message, and
// needs no backend/API cost at all. Once the AI is funded, this
// is skipped automatically — sendMessage() always tries the real
// AI first.
// ============================================================
var faqs = {
...
function addTypingIndicator() {
  ...
  div.style.cssText = "align-self:flex-start;background:#1a1a1a;color:#888;padding:.5rem .9rem;border-radius:12px 12px 12px 2px;font-size:.83rem;font-family:Jost,sans-serif;letter-spacing:2px;";
  div.textContent = "...";
  el.appendChild(div);
  el.scrollTop = el.scrollHeight;
}
...
async function sendMessage(text) {
  ...
  try {
    ...
    if (!res.ok) {
      // AI call failed (e.g. no API credit yet) — answer locally
      // instead of showing an error, so the customer never notices.
      addMsg("assistant", getLocalReply(text));
    ...
    // Network-level failure (site down, no connection) — local
    // fallback still gives a useful answer instead of nothing.
    removeTypingIndicator();
    addMsg("assistant", getLocalReply(text));
  }
...
btn.addEventListener("click", function() {
  ...
  if (chatOpen && document.getElementById("jl-chat-messages").children.length === 0) {
    addMsg("assistant", "Hi! Welcome to Janelle Luxury Hairs. I can help you find the right wig, bundle, closure or accessory. What are you looking for today?");
  }
});
