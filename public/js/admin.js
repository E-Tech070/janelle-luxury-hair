...
function paymentStatusBadge(status) {
  if (status === "confirmed")
    return '<span style="background:rgba(46,204,113,.15);color:#2ecc71;padding:.2rem .65rem;border-radius:100px;font-size:.62rem;font-weight:700;text-transform:uppercase;">✅ Payment Confirmed</span>';
  if (status === "proof_submitted")
    return '<span style="background:rgba(241,196,15,.15);color:#f1c40f;padding:.2rem .65rem;border-radius:100px;font-size:.62rem;font-weight:700;text-transform:uppercase;">🧾 Proof Submitted</span>';
...
async function loadOrders(page) {
  ...
  try {
    ...
    if (!orders.length) {
      el.innerHTML = '<p class="admin-empty">No orders on this page.</p>';
    }
    var html = "";
    orders.forEach(function (o) {
      ...
      if (o.paymentProof) {
        proofHtml =
          '<div class="admin-proof-wrap"><p class="admin-proof-label">💳 Payment Receipt:</p>' +
          '<img src="' +
          o.paymentProof +
          '" class="admin-proof-img" alt="Payment proof" onclick="this.classList.toggle(\'expanded\')" />' +
          (o.paymentStatus !== "confirmed"
            ? '<button class="admin-confirm-payment-btn" data-id="' +
              o._id +
      ...
      html +=
        '<div class="admin-order-card"><div class="admin-order-header"><div class="admin-order-info">';
      html +=
        "<h4>" +
        o.userName +
        "</h4><p>" +
      ...
      html +=
        '<div class="admin-order-total">Total: ₦' +
    ...
    document
      .querySelectorAll(".admin-confirm-payment-btn")
...
// Shared Prev/Next pagination control, reused by any admin list that
// gets paginated (orders now, others can call this the same way).
// onPageChange is called with the page number to switch to.
function renderPagination(containerId, currentPage, totalPages, onPageChange) {
  ...
  var html =
    '<button class="admin-page-btn" id="' +
    containerId +
    '-prev"' +
    (currentPage <= 1 ? " disabled" : "") +
    ">← Prev</button>";
  html +=
    '<span class="admin-page-status">Page ' +
    currentPage +
    " of " +
    totalPages +
    "</span>";
...
function chatBubbleHtml(m) {
  ...
  if (m.photo)
    html +=
      '<img class="chat-bubble-photo" src="' + m.photo + '" alt="Photo" />';
  if (m.text)
...
async function loadConversations() {
  ...
  try {
    ...
    var totalUnread = convos.reduce(function (s, c) {
      return s + c.unreadCount;
    }, 0);
    document.getElementById("unread-badge").textContent = totalUnread;
    document.getElementById("chat-conversations-count").textContent =
      convos.length + " conversations";
    var el = document.getElementById("admin-chat-list");
    if (!convos.length) {
      el.innerHTML = '<p class="admin-empty">No conversations yet.</p>';
      return;
    }
    el.innerHTML = convos
      .map(function (c) {
        var preview = c.lastMessage.text
          ? escapeHtml(c.lastMessage.text).substring(0, 50)
          : "📷 Photo";
        return (
          '<div class="admin-chat-list-item' +
          (currentCustomerId === String(c.customerId) ? " active" : "") +
          '" data-id="' +
          c.customerId +
          '">' +
          '<div class="admin-chat-list-top"><span>' +
          ...
          (c.unreadCount > 0
            ...
            : "") +
          "</div>" +
          '<div class="admin-chat-list-preview">' +
          preview +
          "</div>" +
          '<div class="admin-chat-list-time">' +
    ...
    el.querySelectorAll(".admin-chat-list-item").forEach(function (item) {
      item.addEventListener("click", function () {
        el.querySelectorAll(".admin-chat-list-item").forEach(function (i) {
    ...
    document.getElementById("admin-chat-list").innerHTML =
      '<p class="admin-empty">Could not load conversations.</p>';
  }
}
...
async function openThread(customerId) {
  ...
  try {
    ...
    if (!messages.length) {
      ...
      var wasAtBottom =
        threadEl.scrollTop + threadEl.clientHeight >=
        threadEl.scrollHeight - 30;
      threadEl.innerHTML = messages.map(chatBubbleHtml).join("");
      if (wasAtBottom || threadEl.scrollTop === 0)
        threadEl.scrollTop = threadEl.scrollHeight;
    }
    loadConversations();
  } catch (err) {
    threadEl.innerHTML = '<p class="admin-empty">Could not load thread.</p>';
  }
}
...
document
  .getElementById("admin-chat-photo-btn")
  .addEventListener("click", function () {
    document.getElementById("admin-chat-photo-input").click();
  });
document
  .getElementById("admin-chat-photo-input")
  .addEventListener("change", function () {
    ...
    resizePhotoFile(file, function (dataUrl) {
      ...
      document.getElementById("admin-chat-photo-preview").style.display =
        "flex";
    });
  });
document
  .getElementById("admin-chat-photo-remove-btn")
  .addEventListener("click", function () {
...
async function sendAdminReply() {
  ...
  try {
    ...
    console.log("Could not send reply:", err);
  } finally {
...
document
  .getElementById("admin-chat-send-btn")
  .addEventListener("click", sendAdminReply);
document
  .getElementById("admin-chat-text-input")
...
async function loadReviews() {
  ...
  try {
    ...
    document.getElementById("pending-reviews-badge").textContent =
      reviews.length;
    document.getElementById("reviews-count").textContent =
      reviews.length + " pending";
    var el = document.getElementById("admin-reviews-list");
    if (!reviews.length) {
      el.innerHTML = '<p class="admin-empty">No pending reviews.</p>';
      return;
    }
    el.innerHTML = reviews
      .map(function (r) {
        return (
          '<div class="admin-review-card pending" data-id="' +
          r._id +
          '">' +
          '<div class="admin-review-top"><span><strong>' +
          ...
          "</span></div>" +
          (r.photo
            ? '<img class="admin-review-photo" src="' +
              r.photo +
              '" alt="Review photo" />'
            : "") +
          ...
          '<button class="admin-review-approve" data-id="' +
          r._id +
          '">✅ Approve</button>' +
          '<button class="admin-review-reject" data-id="' +
          r._id +
          '">✕ Reject</button>' +
          "</div></div>"
        );
    ...
    el.querySelectorAll(".admin-review-approve").forEach(function (btn) {
      btn.addEventListener("click", async function () {
        await fetch(API + "/reviews/" + btn.dataset.id + "/approve", {
          method: "PATCH",
          headers: { Authorization: "Bearer " + token },
        });
        loadReviews();
    ...
    el.querySelectorAll(".admin-review-reject").forEach(function (btn) {
      btn.addEventListener("click", async function () {
        await fetch(API + "/reviews/" + btn.dataset.id + "/reject", {
          method: "PATCH",
          headers: { Authorization: "Bearer " + token },
        });
        loadReviews();
    ...
    document.getElementById("admin-reviews-list").innerHTML =
      '<p class="admin-empty">Could not load reviews.</p>';
  }
}
...
async function loadProducts() {
  ...
  try {
    ...
    if (!products.length) {
      el.innerHTML = '<p class="admin-empty">No products yet. Add one!</p>';
      return;
    }
    el.innerHTML = products
      .map(function (p) {
        return (
          ...
          escapeHtml(p.name) +
          (p.active === false
            ? ' <span style="color:#e74c3c;font-size:.65rem;">(hidden)</span>'
            : "") +
          "</div>" +
          ...
          (p.badge ? " · " + escapeHtml(p.badge) : "") +
          "</div>" +
          "</div>" +
          '<div class="admin-product-actions">' +
          '<button class="admin-product-edit-btn" data-id="' +
          p.id +
          '">✏️ Edit</button>' +
          '<button class="admin-product-delete-btn" data-id="' +
          p.id +
          '">🗑️ Delete</button>' +
          "</div></div>"
        );
    ...
    document
      .querySelectorAll(".admin-product-edit-btn")
      .forEach(function (btn) {
        btn.addEventListener("click", function () {
          var p = products.find(function (prod) {
            return prod.id === btn.dataset.id;
          });
          if (!p) return;
          ...
          document.getElementById("pf-oldPrice").value = p.oldPrice
            ? String(p.oldPrice).replace(/[^0-9]/g, "")
            : "";
          ...
          document.getElementById("pf-specs").value = (p.specs || [])
            ...
            .join("\n");
          document.getElementById("product-modal-overlay").style.display =
            "flex";
        });
      });

    document
      .querySelectorAll(".admin-product-delete-btn")
      .forEach(function (btn) {
        btn.addEventListener("click", async function () {
          if (
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    var payload = {
      ...
      oldPrice: document.getElementById("pf-oldPrice").value
        ? parseInt(document.getElementById("pf-oldPrice").value)
        : null,
