...
router.post("/send", authMiddleware, validateMessageBody, async function(req, res) {
  try {
...
router.get("/my", authMiddleware, async function(req, res) {
  try {
    ...
    var messages = await ChatMessage.find({ customer: req.user.id }).sort({ createdAt: 1 });
    res.json(messages);
  } catch(err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
router.get("/my/unread-count", authMiddleware, async function(req, res) {
  try {
    var count = await ChatMessage.countDocuments({ customer: req.user.id, sender: "admin", readByCustomer: false });
...
router.get("/conversations", authMiddleware, adminOnly, async function(req, res) {
  try {
    ...
    var customerIds = conversations.map(function(c) { return c._id; });
...
router.get("/unread-count", authMiddleware, adminOnly, async function(req, res) {
  try {
    var count = await ChatMessage.countDocuments({ sender: "customer", readByAdmin: false });
    res.json({ count: count });
  } catch(err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
router.get("/:customerId", authMiddleware, adminOnly, async function(req, res) {
  try {
    await ChatMessage.updateMany(
      { customer: req.params.customerId, sender: "customer", readByAdmin: false },
      { readByAdmin: true }
    );
    var messages = await ChatMessage.find({ customer: req.params.customerId }).sort({ createdAt: 1 });
    res.json(messages);
  } catch(err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
router.post("/:customerId/send", authMiddleware, adminOnly, validateMessageBody, async function(req, res) {
  try {
