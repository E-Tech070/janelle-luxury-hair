...
var FROM_EMAIL = process.env.FROM_EMAIL || "Janelle Luxury Hairs <onboarding@resend.dev>";
...
function buildOrderEmailHtml(order) {
  ...
  order.items.forEach(function(item) {
    itemsHtml +=
      '<tr>' +
        '<td style="padding:8px 0;color:#e8e8e8;">' + item.name + ' x' + item.qty + '</td>' +
        '<td style="padding:8px 0;text-align:right;color:#c9a84c;">' + item.price + '</td>' +
      '</tr>';
  });

  var deliveryLine = order.deliveryMethod === "pickup"
    ? "🛍️ Self-Pickup (Lagos) — we'll send the pickup address to your phone once confirmed."
    : "🚚 Delivery to: " + (order.shippingAddress ? order.shippingAddress.address + ", " + order.shippingAddress.city + ", " + order.shippingAddress.state : "");
  ...
  return (
    '<div style="background:#0d0d0d;padding:2rem;font-family:sans-serif;">' +
      '<div style="max-width:480px;margin:0 auto;background:#161616;border:1px solid #2a2a2a;border-radius:12px;padding:2rem;color:#e8e8e8;">' +
        ...
        '<p>Hi ' + order.userName + ', we\'ve received your order and it\'s being processed.</p>' +
        '<p style="color:#888;font-size:14px;">Order #' + String(order._id).slice(-6).toUpperCase() + '</p>' +
        '<table style="width:100%;border-collapse:collapse;margin:1.5rem 0;">' + itemsHtml + '</table>' +
        '<div style="border-top:1px solid #2a2a2a;padding-top:1rem;font-weight:bold;color:#c9a84c;">Total: ' + formatMoney(order.total) + '</div>' +
        '<p style="margin-top:1.5rem;font-size:14px;color:#ccc;">' + deliveryLine + '</p>' +
...
async function sendOrderConfirmationEmail(order) {
  ...
  if (!resend) {
    console.log("⚠️  RESEND_API_KEY not set — skipping order confirmation email.");
    return;
  }
  try {
